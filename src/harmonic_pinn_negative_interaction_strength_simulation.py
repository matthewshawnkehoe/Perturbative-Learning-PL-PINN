...
class GrossPitaevskiiPINN(nn.Module):
    ...
    def __init__(self, layers, hbar=1.0, m=1.0, mode=0, eta=1.0, use_perturbation=True):
        """
        Parameters
        ----------
        layers : list of int
        ...
        self.eta = eta  # Interaction strength parameter
    ...
    def pde_loss(self, inputs, predictions, eta, p, potential_type="harmonic", precomputed_potential=None):
        """
    ...
    def normalization_loss(self, u, dx):
        """
        Compute normalization loss using numerical integration.
        """
        integral = torch.sum(u ** 2) * dx
        return (integral - 1.0) ** 2


def train_gpe_model(eta_values, modes, p, X_train, lb, ub, layers,
                    epochs, tol, perturb_const,
                    potential_type='harmonic', lr=1e-5, verbose=True):
    """
    Train the GPE model for different modes and eta values for the PL-PINN implementation.
    ...
    tuple: (models_by_mode, lambda_table, training_history)
        Trained models organized by mode and eta, lambda values, and training histories
    """
    ...
    # Track models, lambda values, training history, and epochs until stopping
    models_by_mode = {}
    lambda_table = {}
    training_history = {}
    constant_history = {}
    ...
    for mode in modes:
        ...
        for eta in reversed(eta_values):
            ...
            if verbose:
                print(f"\nTraining for η = {eta:.2f}, mode = {mode}, nonlinearity p = {p}")

            # Initialize model for this mode and eta
            model = GrossPitaevskiiPINN(layers, mode=mode, eta=eta).to(device)

            # If this isn't the first eta value, initialize with previous model's weights
            if prev_model is not None:
                ...
                # Use advanced initialization for any other starting eta not equal 0
            ...
            for epoch in range(epochs):
                ...
                pde_loss, lambda_value = model.pde_loss(X_tensor, u_pred, eta, p, potential_type)
            ...
            # Record final eigenvalue and save model
            final_lambda = lambda_history[-1] if lambda_history else 0
            lambda_logs.append((eta, final_lambda))
            models_by_eta[eta] = model
            ...
            epochs_by_eta[eta] = final_epoch

            # Update prev_model for next eta value
            prev_model = model

        # Store results for this mode
        lambda_table[mode] = lambda_logs
        models_by_mode[mode] = models_by_eta
        training_history[mode] = history_by_eta
        epochs_history[mode] = epochs_by_eta

    return models_by_mode, lambda_table, training_history, constant_history, epochs_history


def plot_wavefunction(models_by_mode, X_test, eta_values,
                      modes, p, constant_history, perturb_const, potential_type,
                      save_dir="Gross-Pitaevskii/src/final/refine/test"):
    """
    Plot wavefunctions for different modes and eta values.
    """
    os.makedirs(save_dir, exist_ok=True)

    # Map negative eta values to old positive eta indices for consistent coloring
    ...
    for mode in modes:
        ...
        # Plot solutions for different eta values
        for j, eta in enumerate(eta_values):
            if eta not in models_by_mode[mode]:
                continue

            model = models_by_mode[mode][eta]
            model.eval()
            ...
            with torch.no_grad():
                ...
                if abs(eta) % 4 == 0:
                    # Get the old index for this eta value
                    old_j = gamma_to_old_j.get(eta, j)
...
def plot_lambda_vs_eta(lambda_table, modes, p, potential_type, save_dir="Gross-Pitaevskii/src/final/refine/test",
                     sample_interval=4):
    """
    Plot lambda vs. interaction strength for different modes.
    ...
    for i, mode in enumerate(modes):
        ...
        eta_list, lambda_list = zip(*lambda_table[mode])

        # Sample data points at specified interval
        if sample_interval > 1:
            # Take every sample_interval-th point
            sampled_indices = range(0, len(eta_list), sample_interval)
            eta_sampled = [eta_list[idx] for idx in sampled_indices]
            lambda_sampled = [lambda_list[idx] for idx in sampled_indices]

            # Always include the last point if it's not already included
            if len(eta_list) - 1 not in sampled_indices:
                eta_sampled.append(eta_list[-1])
                lambda_sampled.append(lambda_list[-1])
        else:
            eta_sampled = eta_list
            lambda_sampled = lambda_list

        plt.plot(lambda_sampled, eta_sampled,
    ...
    if sample_interval > 1:
        filename = f"lambda_vs_eta_all_modes_p{p}_{potential_type}_sampled_{sample_interval}.png"
    else:
        filename = f"lambda_vs_eta_all_modes_p{p}_{potential_type}.png"
...
def plot_mode0_eta_loss_visualization(training_history, eta_values_to_plot, epochs, p, potential_type,
                                        save_dir="Gross-Pitaevskii/src/final/refine/test"):
    """
    Creates a visualization of the training progress for Mode 0 across different eta values.

    Parameters:
    -----------
    training_history : dict
        Dictionary containing training history for all modes and eta values
    eta_values_to_plot : list
        List of eta values to include in the plot (e.g., [0, -4, -8, -12, -16, -20])
    epochs : int
    ...
    # Plot training progress for Mode 0 across different eta values
    plt.figure(figsize=(12, 8))

    # Restrict number of etas plotted
    eta_values_to_plot = [eta for eta in eta_values_to_plot if eta % 4 == 0]

    # Set up colormap for different eta values
    ...
    filename = f"mode0_eta_loss_comparison_p{p}_{potential_type}.png"
...
def save_models(models_by_mode, lambda_table, training_history, constant_history, epochs_history,
    ...
    for mode in models_by_mode:
        ...
        for eta, model in models_by_mode[mode].items():
            models_state_dicts[mode][eta] = {
...
def load_models(filename="gpe_models.pkl", save_dir="Gross-Pitaevskii/src/final/refine/test"):
    ...
    for mode in data['models_state_dicts']:
        models_by_mode[mode] = {}
        for eta, model_data in data['models_state_dicts'][mode].items():
            # Recreate model
...
if __name__ == "__main__":
    ...
    alpha = 0.25
    # eta_values = [k * alpha for k in range(201)]
    eta_values = [-k * alpha for k in range(81)]
    ...
    for p in nonlinearity_powers:
        ...
        filename = f"{potential_type}_negative_interaction_strength_p{p}_pert_const_{perturb_const}_tol_{tol}_pinn.pkl"

        # Create plotting and model saving directory
        p_save_dir = f"{potential_type}_negative_interaction_strength_pinn_simulations_p{p}"
        ...
        if train_new:
            ...
            models_by_mode, lambda_table, training_history, constant_history, epochs_history = train_gpe_model(
                eta_values, modes, p, X, lb, ub, layers, epochs, tol, perturb_const,
                potential_type='harmonic', lr=1e-4, verbose=True)

            # Save results
            save_models(models_by_mode, lambda_table, training_history, constant_history, epochs_history, filename,
                        p_save_dir)
        else:
            # Load existing models
            print("Loading existing models...")
            models_by_mode, lambda_table, training_history, constant_history, epochs_history = load_models(filename,
                                                                                                       p_save_dir)

        # Plot wavefunctions for individual modes
        print("Generating individual mode plots...")
        plot_wavefunction(models_by_mode, X_test, eta_values, modes, p, constant_history, perturb_const,
