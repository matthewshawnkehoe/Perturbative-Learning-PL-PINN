...
class GrossPitaevskiiPINN(nn.Module):
    ...
    def __init__(self, layers, hbar=1.0, m=1.0, mode=0, eta=1.0, use_residual=True, use_perturbation=True):
        """
        Parameters
        ----------
        layers : list of int
    ...
    def get_complete_solution_with_derivatives(self, x, perturbation, mode=None):
        """
        Get complete solution and its derivatives Returns: (u, u_x, u_xx) where u = base + perturbation
        """
        if mode is None:
            mode = self.mode
    ...
    def pde_loss(self, inputs, predictions, eta, p, potential_type="gravity_well", precomputed_potential=None):
        """
...
def train_gpe_model(eta_values, modes, p, X_train, lb, ub, layers,
    ...
    for mode in modes:
        ...
        for eta in eta_values:
            ...
            mu_logs.append((eta, final_lambda))
        ...
        for j, eta in enumerate(eta_values):
            ...
            if eta % 20 == 0:
                ...
                with torch.no_grad():
                    ...
                    gamma_idx = int(eta / 20)  # 0, 1, 2, 3, 4, 5 for eta=0,20,40,60,80,100
...
def plot_mode0_gamma_loss_visualization(training_history, eta_values_to_plot, epochs, p, potential_type,
                                        save_dir="Gross-Pitaevskii/src/final/refine/test"):
    """
...
if __name__ == "__main__":
    ...
    alpha = 0.25
    eta_values = [k * alpha for k in range(401)]
    #eta_values = [k * alpha for k in range(51)]
    ...
    for p in nonlinearity_powers:
        ...
        # plot_mode0_gamma_loss_visualization(training_history, eta_values, epochs, p, potential_type, p_save_dir)
