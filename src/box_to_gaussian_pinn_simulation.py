...
class GrossPitaevskiiPINN(nn.Module):
    ...
    def __init__(self, layers, hbar=1.0, m=1.0, mode=0, eta=1.0, L=1.0, use_residual=True):
        """
        Parameters
        ----------
        layers : list of int
    ...
    def pde_loss(self, inputs, predictions, eta, p, potential_type="gaussian", precomputed_potential=None):
        """
...
def train_gpe_model(eta_values, modes, p, X_train, lb, ub, layers,
    ...
    """
    Train the GPE model for different modes and eta values.
...
def plot_lambda_vs_eta(lambda_table, modes, p, potential_type, save_dir="Gross-Pitaevskii/src/final/refine/test"):
    ...
    plt.title(f"Eigenvalue vs. Interaction Strength for All Modes (p={p})", fontsize=18)
    plt.grid(True)
    plt.legend(fontsize=12)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"lambda_vs_eta_all_modes_p{p}_{potential_type}.png"), dpi=300)
    plt.close()
